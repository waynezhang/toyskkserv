...
pub fn init(alloc: std.mem.Allocator, ast: std.zig.Ast) !Self {
    ...
    var dict_arr = std.ArrayList(dict.Location).init(alloc);
...
fn loadLocation(alloc: std.mem.Allocator, ast: std.zig.Ast, index: i16) !dict.Location {
...
fn parseConfig(allocator: std.mem.Allocator, files: []const []const u8) !*Self {
    for (files) |file| {
        ...
        const cfg = try allocator.create(Self);
        cfg.* = try Self.init(allocator, ast);
        return cfg;
    }
